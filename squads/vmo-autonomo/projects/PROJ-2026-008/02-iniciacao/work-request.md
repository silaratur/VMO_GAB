...
- Como este projeto é **configuração/desenvolvimento sobre uma plataforma já implantada e validada em outra empresa do grupo** (não uma implantação do zero), o contratante trabalha com uma expectativa de prazo mais curta que a média de mercado para implantações completas — ver Seção 6.
...
**Importante:** o contratante ainda não possui cronograma detalhado (WBS) — este será construído internamente e negociado com o fornecedor selecionado após a contratação. O cronograma abaixo é uma expectativa de alto nível, construída a partir (a) do dimensionamento interno de esforço do projeto e (b) dos benchmarks de mercado pesquisados (Seção 2). Ele é **relativo ao marco T0 (kick-off / assinatura do contrato)**, já que a data de início real ainda depende de designação formal do GP e resolução de pendências internas — **nenhuma data de calendário fixa está sendo assumida neste WR.**

| Marco | Descrição | Prazo estimado (relativo a T0) |
|---|---|---|
| M0 | Kick-off do projeto com o fornecedor selecionado | T0 |
| M1 | Fechamento do levantamento de requisitos detalhado (3 frentes) | T0 + 2 semanas |
| M2 | Entrega da configuração/desenvolvimento — Frente Financeiro | T0 + 4 semanas |
| M3 | Entrega da configuração/desenvolvimento — Frente Suprimentos | T0 + 5 semanas |
| M4 | Entrega da configuração/desenvolvimento — Frente Riscos/Desempenho | T0 + 5,5 semanas |
| M5 | Testes e homologação integrada (UAT) — 3 frentes | T0 + 6 semanas |
| M6 | Go-live das 3 frentes | T0 + 7 semanas |
| M7 | Encerramento do período de suporte pós-go-live (garantia, Seção 9) | T0 + 7 semanas + período de garantia |

**Nota de calibração de mercado:** o prazo interno de referência do contratante (T0 + 7 semanas úteis) é mais curto que a média de mercado observada para implantações completas de sistemas de gestão financeira (tipicamente 90 dias a 12 meses). Essa diferença é considerada compatível pelo contratante porque o escopo desta contratação é **configuração/desenvolvimento sobre uma plataforma já implantada e validada em outra empresa do grupo** (VIX) — não uma implantação do zero. Ainda assim, o fornecedor deve avaliar criticamente essa expectativa em sua proposta e declarar se a considera exequível para o escopo desta Seção 4.1, propondo ajuste justificado caso julgue necessário. Propostas de prazo devem vir acompanhadas de cronograma macro próprio (ver Artefato Obrigatório, Grupo 8).
...
- **Reajuste:** não se aplica reajuste de valores dentro do prazo de execução previsto neste WR (até 7 semanas úteis + garantia), dado o horizonte curto do contrato; reajuste só é aplicável em caso de prorrogação superior a 12 meses, pelo índice a acordar em contrato.
