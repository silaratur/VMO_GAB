...
- **Propagação de correção de prazo ao Work Request (validado 2026-07-07 no PROJ-2026-008):** Quando o prazo do TAP é corrigido/reconciliado após o Cronograma detalhado (Step 17, Carlos Cronograma), a correção NÃO se propaga automaticamente ao Work Request (Step 14, Fábio Fornecedor) — o WR é emitido ANTES do cronograma existir e usa apenas o prazo provisório do TAP na época. Gabriel Governança (Step 25) é quem primeiro detecta essa divergência, tarde demais no pipeline. **Ação preventiva para runs futuros:** sempre que o Oscar aprovar uma correção de prazo no TAP (Step 11/17-ORQ), verificar imediatamente se o Work Request já foi emitido e, se sim, redirecionar também o Fábio Fornecedor para atualizar a Seção 6 (Cronograma Esperado) e cláusulas comerciais dependentes de prazo — não esperar a auditoria de governança para descobrir a divergência.

- **Início de novo run com run anterior pausado em checkpoint (validado 2026-07-07 no PROJ-2026-008):** Ao iniciar um novo run enquanto `squads/vmo-autonomo/state.json` já contém um checkpoint em aberto de outro projeto (ex: aguardando resposta do usuário), o novo run NÃO deve sobrescrever esse arquivo sem antes copiá-lo para `projects/{PROJ-CODE-antigo}/state-paused-checkpoint{N}.json`. Isso preserva o `resume_instructions` do projeto pausado sem impedir o início do novo run.
